...
async function verifyFile(path, kind) {
  ...
  demand(
    formats.some(([mime, match]) => match && meta.contentType === mime),
...
const service = createService({
  ...
  qrCode: (payload) =>
    QRCode.toDataURL(payload, { errorCorrectionLevel: 'M', width: 256, margin: 2 }),
  verifyFile,
});
const emulator = process.env.FUNCTIONS_EMULATOR === 'true';
function endpoint(name, { publicAccess = false, needsTicketKey = false } = {}) {
  return onCall(
    {
      region: 'europe-west1',
    ...
    async (request) => {
      try {
        ...
        if (request.auth) {
          ...
          if (user.disabled || Number(request.auth.token.auth_time) * 1000 < revokedAt)
        ...
        await service.rateLimit(checked, name, name === 'createOrder' ? 15 : 120);
        ...
        console.error('Operation failed', { operation: name, code: error.code || 'unknown' });
...
export const submitRsvp = endpoint('submitRsvp', { publicAccess: true, needsTicketKey: true });
